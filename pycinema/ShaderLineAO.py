...
void main(){
    ...
    vec3 c0 = computeLineAO(uv + vec2(     dx, 3.0*dy) );
    vec3 c1 = computeLineAO(uv + vec2(-3.0*dx,     dy) );
    vec3 c2 = computeLineAO(uv + vec2( 3.0*dx,    -dy) );
    vec3 c3 = computeLineAO(uv + vec2(    -dx,-3.0*dy) );
